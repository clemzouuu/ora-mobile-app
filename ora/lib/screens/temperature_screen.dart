...
class TemperatureScreen extends StatefulWidget {
  final String
  currentTemp; // Cette valeur vient du flux MQTT de StatisticsScreen
  const TemperatureScreen({super.key, required this.currentTemp});
...
class _TemperatureScreenState extends State<TemperatureScreen> {
  ...
  Future<void> fetchHistory() async {
    try {
      final response = await http.get(
        Uri.parse('http://localhost:3000/api/measurements/type/temperature'),
      );
  ...
  Map<String, dynamic> getStatus(double temp) {
    if (temp < 35.0) {
      return {
        "color": Colors.blue,
        "label": "Trop froid",
  ...
  Widget build(BuildContext context) {
    ...
    // 2. Donnée actuelle (MQTT)
    // On essaie de parser la valeur MQTT, sinon on fallback sur la DB
    final double liveValue =
        double.tryParse(widget.currentTemp) ?? dbLatestValue;
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Détails Température"),
        leading: IconButton(
      ...
      body: isLoading
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                children: [
                  // --- CARRÉ TEMPÉRATURE ACTUELLE (MQTT LIVE) ---
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(
                          "${liveValue.toStringAsFixed(1)}°C",
                          style: TextStyle(
                            fontSize: 32,
                            fontWeight: FontWeight.w900,
                            color: getStatus(
                              liveValue,
                            )['color'], // Couleur basée sur le live
                          ),
                        ),
                      ],
                    ),
                  ...
                  // --- SECTION : DERNIÈRE MESURE EN BASE (DIAGNOSTIC) ---
                  const Text(
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        Text(
                          "${dbLatestValue.toStringAsFixed(1)}°C",
                          style: TextStyle(
                            fontSize: 70,
                            fontWeight: FontWeight.w900,
                            color: status['color'],
